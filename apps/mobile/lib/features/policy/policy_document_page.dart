...
typedef ExternalPolicyLauncher = Future<bool> Function(Uri uri);
...
class PolicyDocumentPage extends StatelessWidget {
  const PolicyDocumentPage({
    super.key,
    required this.args,
    this.launchExternal = _defaultLaunchExternal,
  });

  static const routeName = '/policy';

  final PolicyDocumentRouteArgs args;
  final ExternalPolicyLauncher launchExternal;

  @override
  Widget build(BuildContext context) {
    final doc = policyDocumentSpec(args.type);
    return Scaffold(
  ...
  Future<void> _openExternal(BuildContext context, Uri uri) async {
    final opened = await launchExternal(uri);
...
Future<bool> _defaultLaunchExternal(Uri uri) {
...
class PolicyDocumentSpec {
  ...
  final List<PolicyDocumentSection> sections;
}
...
PolicyDocumentSpec policyDocumentSpec(PolicyDocumentType type) {
  switch (type) {
